...
//   2. Compute SHA-256 hash of the FULL object (header + dat//   3. Check if object already exists (deduplication) — if so, just return success
...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    // Step 1: Build the full object with header
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // Step 5: Extract data portion (after null terminator)
    ...
    // Step 6: Allocate and copy data
    *data_out = malloc(actual_data_len + 1);  // +1 for null terminator
    if (!*data_out) {
        free(file_data);
        return -1;
    }
    ...
    ((char *)*data_out)[actual_data_len] = '\0';  // null-terminate for safe string ops
